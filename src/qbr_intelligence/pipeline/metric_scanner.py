...
class MetricScanner:
    ...
    def _build_text_sources(self, artifacts: MetricScanArtifacts) -> list[dict]:
        ...
        for slide in artifacts.slides:
            raw_text = _strip_notes_block(slide.get("raw_text", ""))
            sources.append(
                {
                    "text": raw_text,
...
_NOTES_BLOCK_RE = re.compile(r"### Notes:\s*.*", re.IGNORECASE | re.DOTALL)


def _strip_notes_block(text: str) -> str:
    if not text:
        return text
    match = _NOTES_BLOCK_RE.search(text)
    if not match:
        return text
...
def build_metric_dictionary() -> MetricDictionary:
    definitions = [
        ...
        MetricDefinition(
            name="Installs",
            patterns=(r"\binstalls?\b", r"total installs?"),
            unit_hint="count",
            category="performance",
            slug="installs",
        ),
        MetricDefinition(
            name="Launches",
            patterns=(r"\blaunches?\b", r"total launches?"),
            unit_hint="count",
            category="performance",
            slug="launches",
        ),
        ...
        MetricDefinition(
            name="Investment",
            patterns=(r"\binvestment\b", r"total investment"),
            unit_hint="currency",
            category="cost",
            slug="investment",
        ),
        MetricDefinition(
            name="Spend",
            patterns=(r"\bspend\b", r"\bspending\b", r"total spend"),
            unit_hint="currency",
            category="cost",
            slug="spend",
        ),
        ...
        MetricDefinition(
            ...
            patterns=(
                r"avg session duration",
                r"average session duration",
                r"session duration",
                r"duration per session",
                r"avg(?:erage)? duration per session",
            ),
            unit_hint="time",
            category="engagement",
        ...
        MetricDefinition(
            ...
            patterns=(
                r"\bfrequency\b",
                r"\bfreq\b",
                r"avg monthly freq(?:uency)?",
                r"average monthly freq(?:uency)?",
                r"monthly freq(?:uency)?",
            ),
            unit_hint="count",
            category="reach",
            slug="frequency",
