...
Emitted by `QBRProcessor._export_extraction_outputs()` and `extract_qbr.py`:
...
## Observed Quality Gaps (From `extraction_output/agentiv_disney`)
...
- Avoid breaking `QBRProcessor.process_document()` and `extract_qbr.py` output.
