...
- **Parser**: `python-pptx` adapter produces canonical slide model (text blocks, tables, notes). Falls back to `extraction_output/*/03_slides_parsed.json` if PPTX has corrupted media.
...
- Cache directory defaults to `extraction_output/.metric_adjudicator_cache`.
